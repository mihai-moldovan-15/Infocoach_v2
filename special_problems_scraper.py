# special_problems_scraper.py
# Acest scraper este identic cu advanced_scraper.py, dar rulează doar pe problemele speciale din lista de mai jos.
...
# ... restul codului identic cu advanced_scraper.py ...
class AdvancedPbinfoScraper:
    ...
    def scrape_problems(self, start_id=1, end_id=25):
        """Scrape problemele cu detecție automată a tipului"""
        ...
        for problem_id in range(start_id, end_id + 1):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Verifică dacă problema conține imagini
                    problem_content = soup.find('article', id='enunt')
                    if problem_content:
                        images = self.extract_images(problem_content)
                        if images:
                    ...
                    # Extrage și salvează problema
                    problem_data = self.extract_problem_data(problem_id)
                    if problem_data:
                        if self.save_problem(problem_data):
                            # Salvează fișierul de debug doar dacă problema a fost salvată cu succes
        ...
        # Salvează lista problemelor cu imagini
...
SPECIAL_PROBLEM_IDS = [910]  # poți adăuga aici orice ID-uri speciale

if __name__ == "__main__":
    scraper = AdvancedPbinfoScraper()
    for pid in SPECIAL_PROBLEM_IDS:
        problem_data = scraper.extract_problem_data(pid)
        if problem_data:
            scraper.save_problem(problem_data)
            print(f"✅ Problema {pid} salvată cu succes!")
        else:
            print(f"❌ Nu s-au putut extrage datele pentru problema {pid}") 